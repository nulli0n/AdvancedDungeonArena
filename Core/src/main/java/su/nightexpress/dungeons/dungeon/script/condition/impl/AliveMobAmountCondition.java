...
public class AliveMobAmountCondition extends MobAmountCondition {
    ...
    public AliveMobAmountCondition(@NotNull NumberComparator comparator, int compareValue, @NotNull MobIdentifier mobId) {
    ...
    public static AliveMobAmountCondition read(@NotNull FileConfig config, @NotNull String path) {
        return read(config, path, AliveMobAmountCondition::new);
    }
    ...
    protected int getDungeonValue(@NotNull DungeonInstance dungeon) {
        return dungeon.getCounters().countAliveMobs(this.identifier);
    }
}
